...
const StudentApplications: React.FC = () => {
  ...
  const { currentUser, currentProfile } = ({} as StudentApplicationsProps); // Destructure props
  ...
  const handleFormSuccess = async (applicationId: string) => {
    setShowApplicationForm(false);
    setSelectedForm(null);
    toast.success('Application submitted successfully!');
    await fetchApplications(); // Re-fetch applications to update the list
  };
  ...
  const fetchApplicationDetails = async (applicationId: string) => {
    ...
    try {
      const { data: responses, error: responsesError } = await supabase
        ...
        .select(`*, form_fields (field_label, field_label_hindi, field_type)`)
  ...
  // Loading state for applications and forms
  const loadingApplications = applications.length === 0 && scholarshipForms.length === 0;
